#ifndef STM32F103C6_EXTI_H
#define STM32F103C6_EXTI_H
...
} EXTI_GPIO_Mapping_t; /// refer to @EXTI_define
...
#define AFIO_GPIO_EXTI_map(x) \
  ((x) == GPIOA   ? 0         \
   : (x) == GPIOB ? 1         \
   : (x) == GPIOC ? 2         \
   : (x) == GPIOD ? 3         \
                  : 4)
#endif /* STM32F103C6_EXTI_H */
